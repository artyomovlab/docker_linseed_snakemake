...
} else if (init_strategy == "SelectXVCAConvex") {
...
} else if (init_strategy == "SelectXConvex") {
  r_tilda <- 0.95
...
} else if (init_strategy == "SelectXSubset") {
  thresh <- 2000
...
} else if (init_strategy == "SelectOmega") {
  for (idx in 1:snakemake@config[["num_inits"]]) {
...
} else if (init_strategy == "SelectOmegaConvex") {
  r_tilda <- 0.95
  if ("r_tilda" %in% names(snakemake@config)) {
    r_tilda <- snakemake@config[["r_tilda"]]
  }
...
} else if (init_strategy == "SelectOmegaSubset") {
  thresh <- 100
  if ("thresh" %in% names(snakemake@config)) {
    thresh <- snakemake@config[["thresh"]]
  }
...
} else if (init_strategy == "SelectCentered") {
  for (idx in 1:snakemake@config[["num_inits"]]) {
...
} else if (init_strategy == "SelectRandom") {
